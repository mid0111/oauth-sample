...
everyauth.facebook
  ...
  .findOrCreateUser(
    function (session, accessToken, accessTokExtra, userMetadata) {
      ...
      User.findOne({ userId: 'facebook' + userMetadata.id }, function(err, user) {
        if(err) {
          promise.fail(err);
        }
        if(user) {
...
everyauth.github
  ...
  .findOrCreateUser(
    function (session, accessToken, accessTokExtra, userMetadata) {
      var promise = this.Promise();
      User.findOne({ userId: 'github' + userMetadata.id }, function(err, user) {
        if(err) {
          promise.fail(err);
        }
        if(user) {
...
everyauth.twitter
  ...
  .findOrCreateUser(
    function (session, accessToken, accessTokExtra, userMetadata) {
      var promise = this.Promise();
      User.findOne({ userId: 'twitter' + userMetadata.id }, function(err, user) {
        if(err) {
          promise.fail(err);
        }
        if(user) {
